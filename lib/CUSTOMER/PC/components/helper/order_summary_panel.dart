        ...
        children: [
                    ...
                    List<int> minutes = List.generate(60, (i) => i)
                        .where((m) =>
                            (year > now.year ||
                                month > now.month ||
                                day > now.day ||
                                compareHour > now.hour) ||
                            m >= now.minute)
                        .toList();

                    // ******************************************************************
                    // 🔥 FIX for DropdownButton assertion error
                    // Ensures selected values exist in the generated lists
                    // ******************************************************************

                    if (!months.contains(month)) month = months.first;
                    if (!days.contains(day)) day = days.first;
                    if (!hours.contains(hour)) hour = hours.first;
                    if (!minutes.contains(minute)) minute = minutes.first;
          ...
          Row(
            children: [
              ...
              GestureDetector(
                onTap: () => cartManager.setPayment('Card'),
                child: Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    border: Border.all(
